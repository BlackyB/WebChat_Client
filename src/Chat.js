...
// Get a random int in the range given
function getRandomInt(min, max) {
  return Math.floor(Math.random() * (max - min)) + min;
}

// Return a random name, used when logging in the first time
...
class Chat extends Component {
  ...
  // Etablish connexion between the app and the remote websocket server
  ...
  // This is the final submit step (Client and WebSocket)
  finalSubmit = message => {
    this.addMessage(message);
  ...
  highlight = message => {
      var new_Message = message.message.replace('/highlight', '');
  ...
  submitMessage = messageString => {
    ...
    const message = { name: this.state.name, message: messageString, highlight: false}
    ...
    } else if (messageString.includes('/highlight ')) {

      this.highlight(message);
      this.finalSubmit(message);

      // Remove the last message sent
  ...
  render() {
    return (
      ...
      {this.state.messages.map((message, index) =>
        <ChatMessage key={index} message={message.message} name={message.name} date={this.state.date} myself={message.name === this.state.name} think={this.state.think} highlight={message.highlight} />,
        )}
        </div>
        </div>
